...
def feature_names_for(engine: str | None) -> list[str]:
    return list(get_engine(normalize_feature_engine(engine)).feature_names)


def num_inputs_for(engine: str | None) -> int:
...
def _build_labels(df: pd.DataFrame, cfg: SmartBSConfig) -> np.ndarray:
    ...
    raise ValueError(
        f"label_mode={mode!r} is retired. Train with triple_barrier so labels "
        "stay independent of the Risk Manager."
    )


def make_datasets(df: pd.DataFrame, cfg: SmartBSConfig, *, symbol: str | None = None):
    engine = normalize_feature_engine(getattr(cfg, "feature_engine", "maribbon"))
    ...
    if cfg.label_mode == "triple_barrier":
        ...
        raise ValueError(f"label_mode={cfg.label_mode!r} is retired; use triple_barrier")
