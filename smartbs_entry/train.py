"""Train SmartBS Entry AI (TCN) and save a checkpoint."""
...
from smartbs_entry.calibrate import fetch_training_frame, fit_model_calibration
from smartbs_entry.checkpoint import checkpoint_spec_fields, promote_checkpoint
from smartbs_entry.config import PAIR_ALIASES, SmartBSConfig, resolve_checkpoint_dir
from smartbs_entry.features import make_datasets, make_multi_asset_datasets, num_inputs_for
from smartbs_entry.model import SmartBSClassifier
...
def train_model(cfg: SmartBSConfig) -> str:
    ...
    for sym in assets:
        ...
        path = os.path.join(resolve_checkpoint_dir(), f"{sym}.pt")
...
def parse_args() -> SmartBSConfig:
    parser = argparse.ArgumentParser(description="Train SmartBS Entry AI (TCN)")
    ...
    parser.add_argument("--feature-engine", default="entry", choices=["entry"])
    ...
    cfg = SmartBSConfig(
        ...
        train_assets=assets,
        feature_engine="entry",
        num_inputs=num_inputs_for("entry"),
